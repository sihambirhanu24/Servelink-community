...
export class SuspensionGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (!user.teacherId) {
      this.logger.warn(`SuspensionGuard: User ${user.sub || user.id} has no teacherId`);
      throw new ForbiddenException('Teacher account required');
    }

    // Check and auto-expire if needed
    const isActive = await this.suspensionService.checkSuspensionExpiration(user.teacherId);

    if (!isActive) {
      // Get suspension details for the error response
      const suspensionStatus = await this.suspensionService.getSuspensionStatus(user.teacherId);
      
      this.logger.log(`SuspensionGuard: Teacher ${user.teacherId} is suspended (blocked access)`);
