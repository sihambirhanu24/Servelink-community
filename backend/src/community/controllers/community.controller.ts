...
export class CommunityController {
  ...
  createPost(@CurrentUser() user, @Body() dto: CreatePostDto) {
    console.log(user);
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: 'Create post from community type context (context-aware)',
  })
  @Post('create-by-type')
  @UseGuards(JwtAuthGuard, VerifiedTeacherGuard, SuspensionGuard)
  createPostByType(@CurrentUser() user: any, @Body() dto: CreatePostByTypeDto) {
  ...
  likePost(@Param('id') postId: string, @Req() req) {
    return this.communityService.likePost(postId, req.user.sub);
  }
  ...
  unlikePost(@Param('id') postId: string, @Req() req) {
    return this.communityService.unlikePost(postId, req.user.sub);
  }
  ...
  createComment(
    ...
    return this.communityService.createComment(req.user.sub, postId, dto);
  }
  ...
  markBestAnswer(@Param('commentId') commentId: string, @Req() req) {
    return this.communityService.markBestAnswer(req.user.sub, commentId);
  }
  ...
  @UseGuards(JwtAuthGuard, VerifiedTeacherGuard)
  markHelpful(@Param('commentId') commentId: string, @Req() req) {
    return this.communityService.markHelpful(req.user.sub, commentId);
  }
  ...
  @ApiOperation({
    summary: 'Get Network Community overview stats + recent content',
  })
  @ApiBearerAuth()
  @UseGuards(JwtAuthGuard)
  ...
  updatePost(@Param('id') id: string, @Body() dto: CreatePostDto, @Req() req) {
    return this.communityService.updatePost(id, req.user.sub, dto);
  }
  ...
  deletePost(@Param('id') id: string, @Req() req) {
    return this.communityService.deletePost(id, req.user.sub);
  }
  ...
  bookmarkPost(@Param('postId') postId: string, @Req() req) {
    return this.communityService.bookmarkPost(req.user.sub, postId);
  }
  ...
  unBookmarkPost(@Param('postId') postId: string, @Req() req) {
    return this.communityService.unBookmarkPost(req.user.sub, postId);
  }
  @ApiBearerAuth()
  @UseGuards(JwtAuthGuard)
  @Get('posts')
  ...
  uploadAttachment(
    ...
    return this.communityService.uploadAttachment(file, postId);
  }
  ...
  deleteAttachment(@Param('attachmentId') attachmentId: string, @Req() req) {
    return this.communityService.deleteAttachment(attachmentId, req.user.sub);
  }
  ...
  @UseGuards(JwtAuthGuard)
  @Get('type/:type')
  getCommunitiesByType(@Param('type') type: string, @CurrentUser() user: any) {
  ...
  getMembersByType(@Param('type') type: string, @CurrentUser() user: any) {
  ...
  getDiscussionMessages(
    ...
    return this.chatService.getDiscussionMessages(
      postId,
  ...
  @UseGuards(JwtAuthGuard)
  @Get(':id')
  getCommunity(@Param('id') id: string, @CurrentUser() user: any) {
