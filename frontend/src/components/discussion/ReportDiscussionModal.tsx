...
import { useReportDiscussion } from "@/hooks/useDiscussions";
import { ReportReason } from "@/types/discussion";

interface ReportDiscussionModalProps {
  discussionId: string;
...
export function ReportDiscussionModal({
  ...
  const handleSubmit = () => {
    ...
    reportMutation.mutate(
      { id: discussionId, dto: { reason: selectedReason, description: description.trim() || undefined } },
      {
        onSuccess: () => {
          setShowSuccess(true);
          setTimeout(() => {
